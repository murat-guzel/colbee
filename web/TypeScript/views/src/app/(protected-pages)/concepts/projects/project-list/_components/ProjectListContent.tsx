...
const ProjectListContent = () => {
    ...
    const getProjectKey = (project: any, prefix: string, index: number) => {
        // Always include index to ensure uniqueness
        return `${prefix}-${project.id || index}-${index}`
    }

    const favoriteProjects = useMemo(() => {
        return projectList?.filter((project) => project.favourite) || []
    }, [projectList])
    ...
    return (
        <div>
            <div className="mt-8">
                {favoriteProjects.length > 0 && <h5 className="mb-3">Favorite</h5>}
                <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {favoriteProjects.map((project, index) => (
                        <Card key={getProjectKey(project, 'favorite', index)} bodyClass="h-full">
            ...
            <div className="mt-8">
                {otherProjects.length > 0 && <h5 className="mb-3">Other projects</h5>}
                <div className="flex flex-col gap-4">
                    {otherProjects.map((project, index) => (
                        <Card key={getProjectKey(project, 'other', index)}>
